$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
            { label: '职位编码', name: 'roleId', index: "role_id", width: 45, key: true },
            { label: '职位名称', name: 'roleName', index: "role_name", width: 75 },
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function () {
            ...
            // console.log(vm.role.deptName);
            vm.postion.deptId = vm.role.deptId;
            console.log(vm.postion.code);
            console.log(vm.postion.name);
            console.log(vm.postion.deptId);
            var url = vm.role.roleId == null ? "sys/position/savePostion" : "sys/position/savePostion";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.postion),
                success: function(r){
                    console.log(r);
                    if(r.code === 0){
                        alert('操作成功', function(){
